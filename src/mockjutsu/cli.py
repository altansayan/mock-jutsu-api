...
# ---------------------------------------------------------------------------
_REFERENCE = [
    ('tckn'           , 'Identity'     , False, '45678901234'           , 'generate tckn'                 , 'Turkish ID (TCKN) with Modulo 10/11 validation.', '--prefix'),
    ...
    ('firstname'      , 'Name'         , True , 'Emre'                  , 'generate firstname --locale TR', 'Random first name for the specified locale.', '--gender'),
    ('lastname'       , 'Name'         , True , 'Yilmaz'                , 'generate lastname --locale TR' , 'Random last name for the specified locale.', '--gender'),
    ('fullname'       , 'Name'         , True , 'Emre Kaya'             , 'generate fullname --locale TR' , 'Full name (First + Last) for the specified locale.', '--gender'),
    ('patronymic'     , 'Name'         , True , 'Ivanovich'             , 'generate patronymic --locale RU', 'Patronymic middle name (common in Russian locale).', '--gender'),
    ...
    ('age'            , 'Demographic'  , False, '34'                    , 'generate age'                  , 'Random human age (typically 18-90).', '--min --max'),
    ...
    ('cardnum'        , 'Financial'    , False, '4532 0151 9283 1029'   , 'generate cardnum --network visa', 'Credit card number with Luhn algorithm validation.', '--network'),
    ...
    ('cardowner'      , 'Financial'    , True , 'JOHN SMITH'            , 'generate cardowner --locale TR', 'Cardholder name in appropriate format for locale.', '--gender'),
    ...
    ('balance'        , 'Financial'    , False, '12450.75'              , 'generate balance'              , 'Random account balance with 2 decimal precision.', '--min --max'),
    ...
    ('3ds_cavv'       , 'Financial'    , False, 'AAABBIIFmAAAAAAEggWY...', 'generate 3ds_cavv'            , '3D Secure 2.0 Cardholder Authentication Verification Value (CAVV).', '--network'),
    ('3ds_eci'        , 'Financial'    , False, '05 (Visa) / 02 (MC)'   , 'generate 3ds_eci --network visa', '3D Secure Electronic Commerce Indicator (ECI) flag.', '--network'),
    ...
    ('hash'           , 'Meta'         , False, 'e3b0c44298fc...(64hex)', 'generate hash --algorithm sha256', 'Cryptographic hash values for various algorithms.', '--algorithm'),
    ...
    ('crypto_address' , 'Crypto'       , False, '(btc or eth)'          , 'generate crypto_address --currency eth', 'Generic crypto address for specified currency.', '--currency'),
    ('tx_hash'        , 'Crypto'       , False, 'a1b2c3...64hex'        , 'generate tx_hash --currency btc', 'Blockchain transaction hash (SHA-256/Keccak-256).', '--currency'),
    ('block_hash'     , 'Crypto'       , False, '0x+64hex (eth)'        , 'generate block_hash --currency eth', 'Blockchain block hash identifier.', '--currency'),
    ...
    ('tracking_number', 'E-Commerce'   , False, '9400111899223397522384', 'generate tracking_number --carrier usps', 'Logistic tracking numbers for major carriers.', '--carrier'),
