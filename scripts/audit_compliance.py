...
def audit():
    print("🛡️ Mock Jutsu Compliance Audit starting...")
    ...
    for rt in ref_types:
        if rt not in core_types and rt != "cardowner":
            errors.append(f"⚠️ [Orphan] Type '{rt}' is in CLI reference but NOT in core registries.")
    ...
    if errors:
        ...
        for err in errors:
            print(err)
        print("\nFix these issues before pushing. Stay professional. ⚔️")
        sys.exit(1)
    else:
